...
%This function runs the parallel calibration algorithm described in Cho,
...
%
%   Inputs:
%       -type: uses synthetic data from cellular automaton model
%               - enter 'Competitive','Mutual', 'RantagC', or 'Neutral'
%
...
function LVcalibration_Parallel_CAData( type )

%Inputs:
% type: Competitive, Mutual, CantagR, RantagC, or Neutral

dataLocation = ['Data/CAData/' type  '/'];
figLocation = ['Figures/Parallel_CAData/' type '/'];
mkdir(figLocation)
...
errorMatrix = zeros(9,9); %keep track of sum of SSEs across all nine data sets
paramsList = zeros(81,8); %keep track of params for each mixture combo
signMatrix = zeros(9,9);
count = 0;
...
for p1 = 1:9
    for p2 = 1:9
        ...
        ctrlprop = 0.1*[p1 p2];
        paramsList(count,1:2) = ctrlprop; %first two entries in param list are the ctrl prop mixtures used
        
        
        if p1==p2
            errorMatrix(p1,p2) = NaN;
            paramsList(count,3:8) = [NaN NaN NaN NaN NaN NaN];
            signMatrix(p1,p2) = NaN;
        else
            conds = 0.02.*[ctrlprop(1) 1-ctrlprop(1); ctrlprop(2) 1-ctrlprop(2)]; %get resistant proportions to match ctrl props
            idx = [1:8; 9:16]; %Specify which data belongs to which data set
            
            %Load high-fidelity ABM data
            filePattern = fullfile(dataLocation,'*.mat');
            myFiles = dir(filePattern);
            
            
            myFiles2(1) = myFiles(p1);
            myFiles2(2) = myFiles(p2);
            ...
            weeklyIdx = [8 15 22 29 36 43 50 57]; %Thin data to be practical: one point per week
            ...
            for i = 2:numMixtures
                baseFileName = myFiles2(i).name;
                fullFileName = fullfile(myFiles2(i).folder,baseFileName);
                load(fullFileName)
                
                timedata = [timedata; data.xdata(weeklyIdx)'];
                tumordata = [tumordata; data.ydata(:,weeklyIdx)'];
            ...
            try
                ...
                resultsFileName = [figLocation 'ChainInfo_Mix1Pt' num2str(p1) '_Mix2Pt' num2str(p2) '.mat'];
                save(resultsFileName,'results','chain','s2chain','ss2chain','bayesparams')
                
                
                paramsList(count,3:8) = bayesparams; %add estimated params to list
                ...
                for k = 1:numMixtures
                    ...
                    subplot(2,1,k)
                    plot(data.xdata(idx(k,:)),data.ydata(idx(k,:),1),'ob')
                    hold on
                    plot(data.xdata(idx(k,:)),data.ydata(idx(k,:),2),'or')
                ...
                figName = [figLocation 'Mix1Pt' num2str(p1) '_Mix2Pt' num2str(p2) '_ModelFits.fig'];
                saveas(gcf, figName)
                
                
                figure(2)
                mcmcplot(chain,[],{'r1','r2','K1','K2','\gamma_S','\gamma_R'},'chainpanel')
                figName = [figLocation 'Mix1Pt' num2str(p1) '_Mix2Pt' num2str(p2) '_Chains.fig'];
                saveas(gcf, figName)
                
                figure(3)
                mcmcplot(chain,[],{'r1','r2','K1','K2','\gamma_S','\gamma_R'},'denspanel')
                figName = [figLocation 'Mix1Pt' num2str(p1) '_Mix2Pt' num2str(p2) '_Densities.fig'];
                saveas(gcf, figName)
                
                figure(4)
                mcmcplot(chain,[],{'r1','r2','K1','K2','\gamma_S','\gamma_R'},'pairs')
                figName = [figLocation 'Mix1Pt' num2str(p1) '_Mix2Pt' num2str(p2) '_PairwisePlots.fig'];
                saveas(gcf, figName)
...
SSE = 0;
conds = 0.02.*[ctrlprop(1) 1-ctrlprop(1); ctrlprop(2) 1-ctrlprop(2)];
idx = [1:8; 9:16]; 
for j = 1:2
    v0 = conds(j,:);
    ...
    if length(volume(:,1))<71
        ...
        SSE = SSE+sum(sum((myData.ydata(idx(j,:),:)-volume(weeklyIdx,:)).^2));
    end
end
end